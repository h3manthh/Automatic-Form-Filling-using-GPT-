...
def process_form_submission(input_file, data_file):
    ...
    output_file_path = f"/Users/hope/Desktop/Final Project/FormFiller/output/{secure_filename(input_file)}_filled.docx"
...
def home():
    if request.method == 'POST':
        ...
        # Render the view template with an option to download the filled document
        return render_template('view.html', output_file_path=output_file_path)

    return render_template('index.html')

# Route to view the filled document
@app.route('/view/<filename>')
def view_filled_document(filename):
    return send_file(f'./output/{filename}', as_attachment=False)

# Route to download the filled document
@app.route('/download/<filename>')
def download_filled_document(filename):
    return send_file(f'./output/{filename}', as_attachment=True)

if __name__ == '__main__':
    # Create the 'uploads', 'temp', and 'output' directories if they don't exist
